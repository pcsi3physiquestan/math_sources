...
_Nous allons être très souvent confrontés à des grandeurs définies en tout l'espace (ou portion). On parlera de __champ__. On peut citer l'énergie potentielle qui peut se définir en tout point de l'espace._


## Champ vectoriel et champ scalaire

````{important} __Champ vectoriel et champ scalaire__  
...
### Dérivées partielles
````{margin} __Variables constantes__  
...
````
````{important} __Dérivées partielles__  
...
_Un déplacement élémentaire $\overrightarrow{dOM}$ correspond à un déplacement infinitésimal (c'est-à-dire tendant vers O) à partir du point M vers un point M': $\overrightarrow{dOM} = \overrightarrow{MM'}_{M' \to M}$._
...
````{topic} Remarques
* __Analogie topographique__ : Il est conseillé pour mieux visualiser les caractéristiques suivantes d'imaginer un champ scalaire z(x,y) qui donnerait l'altitude z en un point de coordonnées (x,y) sur une carte topographique.
* __Orientation et sens du gradient__ : Le gradient est toujours orienté dans la direction où le champ f varie le plus fortement et dans le sens croissant de f (suivant la droite de plus grande pente).
...
* __Courbe iso-f__ : Le gradient est toujours perpendiculaire aux courbes/surfaces où la fonction f est uniforme (f(x,y,z) = cste - les courbes iso-altitude sur une carte topographique).
...
* __Nullité du gradient__ : Un gradient nul implique que chaque dérivée partielles est nulle, c'est-à-dire que la fonction f atteint un extremum suivant chaque coordonnées. On distingue alors plusieurs cas:
...
```
````{important} __Expression du gradient dans les systèmes de coordonnées__  
